...
// ToDo: эта часть будет заменена на бизнес-процесс
...
//
// ПАРАМЕТРЫ
//  КомандаТелеграмБота - Строка
// 
// Возвращаемое значение:
//   HTTPОтвет   - ответ от телеграм-бота на высланную команду
Функция ОбработатьКомандуНаСервереТелеграм(КомандаТелеграмБота)
	
	Ресурс     = Бот.Токен + КомандаТелеграмБота;
	Соединение = Новый HTTPСоединение(Бот.Сервер, 443, , , ,60 , Новый ЗащищенноеСоединениеOpenSSL(Неопределено, Неопределено));
	Запрос     = Новый HTTPЗапрос(Ресурс);
	КоличествоПопыток = 3;
	
	// в связи с перебоями с доступом к серверу телеграм из-за блокировок Роскомнадзора, пробуем несколько раз
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Исключение
			...
			ТекстОшибки = Нстр("ru='Ошибка при соединения с сервером телеграм! 
				|Данные ошибки: %1
				|Детали: бот: %2, кол. попыток: %3, команда: %4'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
				Бот.Наименование, СчетчикПопыток, Лев(КомандаТелеграмБота, 100));

			ЗаписьЖурналаРегистрации(Нстр("ru='ТелеграмБот: ОбработатьКомандуНаСервереТелеграм'"), 

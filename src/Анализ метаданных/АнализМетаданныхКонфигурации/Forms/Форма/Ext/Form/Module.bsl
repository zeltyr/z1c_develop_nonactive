...
Процедура ОбойтиЭлементМетаданных(ДеревоМетаданных, пИмяЭлемента, ЕстьСтандартныйРеквизит)
	...
	Для каждого Элем Из Метаданные[пИмяЭлемента] Цикл
		...
		Если пИмяЭлемента = "Документы" Тогда
			ЗаполнитьДопДанныеПоДокументам(Уровень1, Элем.Имя);
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДопДанныеПоДокументам(Уровень1, пИмяЭлемента)
	
	Попытка
		ДанныеДок = ДанныеДокументов(пИмяЭлемента);
...
Функция ДанныеДокументов(ИмяДокумента)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МАКСИМУМ(Документ.Дата) КАК ДатаПоследнего,
	|	СУММА(ВЫБОР
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Документ.Ссылка) КАК ИтогоДокументов
	|ИЗ
	|	ТабДокумент КАК Документ";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТабДокумент", "Документ." + ИмяДокумента);
...
Функция СформироватьОписаниеРеквизита(Реквизит)
	...
	Если СтрНайти(ЗначениеТипа, "Строка") Тогда
		Описание = "Строка (%1)";
		Описание = СтрШаблон(Описание, Реквизит.Тип.КвалификаторыСтроки.Длина);
		ЗначениеТипа = СтрЗаменить(ЗначениеТипа, "Строка", Описание);
	КонецЕсли;
	
	Если СтрНайти(ЗначениеТипа, "Число") Тогда
		Описание = "Число (%1, %2)";
		Описание = СтрШаблон(Описание, Реквизит.Тип.КвалификаторыЧисла.Разрядность, 
								Реквизит.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти);
		ЗначениеТипа = СтрЗаменить(ЗначениеТипа, "Число", Описание);
	КонецЕсли;
	
	Если СтрНайти(ЗначениеТипа, "Дата") Тогда
		Описание = "Дата (%1)";
		Описание = СтрШаблон(Описание, Реквизит.Тип.КвалификаторыДаты.ЧастиДаты);
